...
const url = 'http://api.openweathermap.org/data/2.5/weather?q=San%20Francisco&APPID=' + apiKey
...
fetch(url).then(function(response) {
    if (response.ok) {
      ...
      return kelvin;
  }).then(function(kelvin){
      const degF = (kelvin - 273.15) * 1.8 + 32;
      const degFInt = Math.floor(degF);
      console.log('The local temp is: ' + degFInt);
      document.getElementById("temp").value(degFInt);
  })

  const apiKey2 = '';
  const appId  = '';
  
  const url2 = 'http://api.weatherunlocked.com/api/current/us.94601?app_id={}}&app_key={}';
